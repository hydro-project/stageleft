...
pub fn q_impl(root: TokenStream, toks: proc_macro2::TokenStream) -> TokenStream {
    ...
    let unitialized_free_variables = visitor.free_variables.iter().map(|i| {
        ...
        quote!(
            ...
            let #i_without_span = {
                ...
                __output.captures.push((#ident_str.to_string(), ::#root::runtime_support::FreeVariableWithContext::to_tokens(#i_without_span, __stageleft_ctx)));
                _out
            };
        )
    });
    ...
    quote!({
        move |__stageleft_ctx: &_, __output: &mut #root::internal::QuotedOutput| {
            ...
            if true {
                __output.module_path = module_path!().to_string();
